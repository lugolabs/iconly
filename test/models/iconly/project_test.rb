...
module Iconly
  class ProjectTest < ActiveSupport::TestCase
    ...
    test 'owned_by returns projects by user' do
      fred_projects = %i(one two three).map { |id| projects(id) }
      assert_equal fred_projects, Project.owned_by(users(:fred))
    end

    test 'without_icons returns projects without icons' do
      projects_without_icons = %i(three fourth).map { |id| projects(id) }
